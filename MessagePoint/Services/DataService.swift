...
class DataService {
    ...
    func getUsername (forUID uid: String, hendler: @escaping (_ username: String) -> ()) {
        REF_USERS.observeSingleEvent(of: .value) { (userSnapshot) in
            guard let userSnapshot = userSnapshot.children.allObjects as? [DataSnapshot] else {return}
            for user in userSnapshot {
                if user.key == uid {
                    hendler(user.childSnapshot(forPath: "email").value as! String)
                }
            }
        }
    }
    ...
    func getIds(forUsernames usernames: [String], handler: @escaping([String]) -> ()) {
        REF_USERS.observeSingleEvent(of: .value) { (userSnapshot) in
            var idArray = [String]()
            guard let userSnapshot = userSnapshot.children.allObjects as? [DataSnapshot] else {return}
            for user in userSnapshot {
